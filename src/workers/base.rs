...
use crate::serum::state::{Order, Side};
...
pub trait BotThread {
    fn worker(&self) {
        ...
        let serum_market_info = connection.get_account(&str_to_pubkey(&config.trader.market_address)).unwrap();
        let mut serum_market_info_clone = serum_market_info.clone();
        let serum_market_account_info = AccountInfo {
            key: &str_to_pubkey(&config.trader.market_address),
            is_signer: false,
            is_writable: false,
            lamports: Rc::new(RefCell::new(&mut serum_market_info_clone.lamports)),
            data: Rc::new(RefCell::new(&mut serum_market_info_clone.data)),
            owner: &serum_market_info.owner().clone(),
            executable: false,
            rent_epoch: serum_market_info.rent_epoch,
        };
        ...
        loop {
            ...
            if ix.len() > 0 {
                ...
                if let Ok(block_hash) = latest_block_hash {
                    ...
                    match sig1 {
                        Ok(sig) => {
                            self.log_transaction_logs_(&connection, &sig);
                        }
                        Err(e) => self.log_rpc_client_error_(e)
                    }
                } else {
                    self.log_rpc_client_error_(latest_block_hash.unwrap_err())
                }
            }
        }
    }
    ...
    fn parse_order_book_for_owner(side: Side, slab: &Slab, owner: &[u64; 4]) -> Vec<Order> {
        ...
        for i in 0..min(slab.capacity(), 40) {
